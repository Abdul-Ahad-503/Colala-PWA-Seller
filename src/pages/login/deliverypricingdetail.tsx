...
const DeliveryPricingDetail: React.FC<DeliveryPricingDetailProps> = ({
  ...
  useEffect(() => {
    ...
    // Load saved data
    const savedPricingData = Cookies.get('registrationDeliveryPricingData');
    if (savedPricingData) {
      try {
      ...
      <div className="w-1/2 flex flex-col bg-[#F9F9F9] max-h-[666px]">
        ...
        <div className="p-6 pb-4 flex-shrink-0">
          ...
          <div className="flex items-center mb-3">
            <button
              onClick={onBack}
              className="mr-4 p-1 hover:bg-gray-100 border border-[#CDCDCD] bg-white rounded-full transition-colors"
            >
              <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <h3 className="text-[18px] font-medium text-[#000000]">Delivery Pricing Details</h3>
          </div>
        </div>
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                <img src="/public/CaretLeft.svg" alt="Dropdown Icon" className="w-5 h-5 mr-62 text-gray-600" />
              </div>
            </div>
          </div>

          {/* Delivery Locations List */}
          <div className="space-y-3 mb-6 w-[380px]">
              ...
              <div key={location.id} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm">
                ...
                <div className="flex gap-2 -mt-10">
                  <button
                    ...
                    className="bg-primary ml-56  text-white   text-[8px] font-medium py-2 px-6 rounded-2xl hover:bg-red-600 transition-colors"
                  >
                    Edit
                  </button>
                  <button
